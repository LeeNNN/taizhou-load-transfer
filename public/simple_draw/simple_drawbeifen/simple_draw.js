draw1 = {
    OPITION: {
        ...
        open_outside_feeder_render: true
    },
    ...
    drawIcon: function (left, top, icon, name, flag, point) {
        ...
        if (name) {
            ...
            var b = "<p  class=\"topo_p\" style = \"top:" + (top + $(a).height() + 10 + icon.height) + ";left:" + left + ";font-size:16px;\">" + name + "</p>"
    ...
    startDraw: function (d, nodeX, nodeY, trueEndY, flag) {
        ...
        if (d.type == this.SUBSTAION_TYPE) {
            ...
            var qH = this.getNodHeight(d.type, d.children)
            var qW = this.BUSBAR_WIDTH
            ...
            if (childNode.length == 1) {
                ...
                if (this.OPITION.open_busbar_breaker_merge) {
                    if (k != 0 && k != undefined) {
                        ...
                        var u = w_size > this.counter * 40 ? w_size - this.counter * 40 : 0
                        this.doDrawLine(startX + u, startY, endX, endY, childNode[0].feederId)
                    ...
                    if (this.OPITION.open_busbar_breaker_merge) {
                        ...
                        if (currentNode.type == this.BUSBAR_TYPE) {
                            ...
                            if (k != 0 && k != undefined) {
                                ...
                                var u = w_size > this.counter * 40 ? w_size - this.counter * 40 : 0
                                this.doDrawLine(startX + u, startY, endX, endY, childNode[i].feederId)
                            } else {
    ...
    doSpecialDrawLine: function (startX, startY, endX, endY, feedId) {
        ...
        this.counter++
        if (startY < endY) {
            var a = "<div class=\"toBottomDiv topo_p " + g + "\" style=\"height:" + (endY - startY) + ";width:" + ((endX - startX) - this.counter * 40) + ";top:" + startY + ";left:" + startX + "\"></div>"
            this.container.append(a)

        } else if (startY == endY) {
            var a = "<div class=\"lineDiv topo_p " + g + "\" style=\"width:" + ((endX - startX) - this.counter * 40) + ";top:" + startY + ";left:" + startX + "\"></div>"
            this.container.append(a)

        } else if (startY > endY) {
            var a = "<div class=\"toTopDiv topo_p " + g + "\" style=\"height:" + (startY - endY) + ";width:" + ((endX - startX) - this.counter * 40) + ";top:" + endY + ";left:" + startX + "\"></div>"
            this.container.append(a)
        }
    ...
    renderCombine: function (node, nodeX, nodeY, chidlNode) {
        if (node.combinedId != undefined && node.combinedId != null && node.combinedId != 0 && node.combinedName.indexOf("分支箱") < 0 && node.feederId == this.CURRENT_FEEDER_ID) {
            var o = []
            var combineInfo = {}
            ...
            if (this.COMBINE_CONTAINER.length == 0) {
                ...
                var f = 0
                for (i = 0; i < this.COMBINE_CONTAINER.length; i++) {
                    for (j = 0; j < this.COMBINE_CONTAINER[i].length; j++)
    ...
    drawCombine: function (startX, startY, endX, endY, name) {
        ...
        var a = "<div class=\"topo_p combine\" style=\"border-style:solid;height:" + (endY - startY) + ";width:" + (endX - startX) + ";top:" + startY + ";left:" + startX + "\"></div>"
        var b = "<p class=\"topo_p\" style=\"top:" + (startY - 100) + ";left:" + startX + ";font-size:16px;\">" + name + "</p>"
        this.container.append(a).append(b)
    }
